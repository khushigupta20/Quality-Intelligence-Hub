...
class DefectAnalyzer:
    def __init__(self, data_file,
        ...
        """Initialize the analyzer with defect data file and optional watsonx credentials."""
    ...
    def run_analysis(self):
        ...
        if status_col:
            ...
            open_count = len(self.df[self.df[status_col].astype(str).str.contains(
        ...
        if sev_col:
            ...
            if len(rca_df) > 0 and id_col and summary_col:
                ...
                # Phase 4 — per-defect root cause suggestions (LLM only)
                if watsonx_agent.is_configured(self.watsonx_api_key, self.watsonx_project_id):
                    self._suggest_root_causes(results['rca_candidates'])
                    results['ai_enhanced'] = True

        self.analysis_results = results
        return results

    def _suggest_root_causes(self, candidates: list) -> None:
        """Add 'suggested_root_cause' field to each RCA candidate (in-place).

        Calls the watsonx LLM for the top 20 candidates only to avoid
        excessive API usage. Silently skips on individual failures.
        """
        import logging
        log = logging.getLogger(__name__)
        for candidate in candidates[:20]:
            summary  = candidate.get('full_summary') or candidate.get('summary', '')
            severity = candidate.get('severity', 'Unknown')
            module   = candidate.get('module', 'General')
            result   = watsonx_agent.suggest_root_cause(
                summary, severity, module,
                self.watsonx_api_key, self.watsonx_project_id, self.watsonx_model_id
            )
            if "error" not in result:
                candidate['suggested_root_cause'] = result['suggestion']
            else:
                log.warning("Root cause suggestion failed for %s: %s",
                            candidate.get('id', '?'), result['error'])
    ...
    def _group_similar_defects(self, df, summary_col, id_col, sev_col, module_col):
        ...
        When watsonx credentials are configured, uses semantic embedding
        similarity for grouping. Falls back to keyword matching.
        """
        from collections import defaultdict
        import re

        # Build list of defect dicts for all rows
        defect_rows = []
        for _, row in df.iterrows():
            defect_rows.append({
                'id':           str(row[id_col]),
                'summary_raw':  str(row[summary_col]),
                'summary_lower':str(row[summary_col]).lower(),
        ...
        # ── Semantic grouping via watsonx embeddings ──────────────────────────
        if watsonx_agent.is_configured(self.watsonx_api_key, self.watsonx_project_id) and defect_rows:
            all_summaries = [d['summary_raw'] for d in defect_rows]
            cluster_result = watsonx_agent.cluster_defects(
                all_summaries, self.watsonx_api_key, self.watsonx_project_id
            )
            if "error" not in cluster_result:
                for gid, indices in cluster_result["groups"].items():
                    first = defect_rows[indices[0]]
                    group_key = f"{first['module']}_cluster{gid}"
                    for idx in indices:
                        d = defect_rows[idx]
                        groups[group_key].append({
                            'id':           d['id'],
                            'summary':      d['summary_raw'][:100],
                            'severity':     d['severity'],
                            'module':       d['module'],
                            'full_summary': d['summary_raw'],
                        })
                # Skip keyword fallback — use semantic groups
                return self._build_representatives(groups)
            else:
                import logging
                logging.getLogger(__name__).warning(
                    "Semantic clustering failed: %s — falling back to keyword grouping",
        ...
        for d in defect_rows:
            matched_keywords = [kw for kw in keywords if kw in d['summary_lower']]
            if matched_keywords:
                group_key = f"{d['module']}_{matched_keywords[0]}"
            else:
                words = re.findall(r'\b\w{5,}\b', d['summary_lower'])
                group_key = f"{d['module']}_{words[0]}" if words else f"{d['module']}_other"
    ...
    def _build_representatives(self, groups):
        """Convert a groups dict into a sorted list of representative defect dicts."""
        representatives = []
    ...
    def _generate_five_whys(self, category, defect_summaries=None):
        ...
        When defect_summaries are provided and watsonx credentials are configured,
        the analysis is generated by the LLM using the actual defect text.
        Falls back to the static templates when watsonx is not configured or the call fails.
        """
        # ── LLM path ──────────────────────────────────────────────────────────
        if defect_summaries and watsonx_agent.is_configured(self.watsonx_api_key, self.watsonx_project_id):
            result = watsonx_agent.generate_five_whys(
                category, defect_summaries,
                self.watsonx_api_key, self.watsonx_project_id, self.watsonx_model_id
            )
            if "error" not in result:
                return result
            # Log and fall through to static template
            import logging
            logging.getLogger(__name__).warning(
                "watsonx 5-Whys failed for category '%s': %s — using static template",
    ...
    def _generate_preventive_measures(self, grouped_defects):
        ...
        When watsonx credentials are configured, category-specific immediate
        actions and long-term prevention are generated by the LLM using the
        actual defect summaries as context. Falls back to static lists on failure.
        """
        measures = []
        ...
        use_llm = watsonx_agent.is_configured(self.watsonx_api_key, self.watsonx_project_id)
        for category, data in categories.items():
            if category in category_measures:
                ...
                # ── 5-Whys (LLM or static) ────────────────────────────────
                five_whys = self._generate_five_whys(category, summaries)

                # ── Immediate actions + long-term (LLM or static) ─────────
                immediate_actions   = measure_info['measures'][:3]
                ...
                if use_llm:
                    llm_result = watsonx_agent.generate_measures(
                        category, summaries, modules_list,
                        self.watsonx_api_key, self.watsonx_project_id, self.watsonx_model_id
                    )
                    if "error" not in llm_result:
                        immediate_actions    = llm_result.get('immediate',  immediate_actions)
                        long_term_prevention = llm_result.get('long_term',  long_term_prevention)
                    else:
                        import logging
                        logging.getLogger(__name__).warning(
                            "watsonx measures failed for '%s': %s — using static",
                            category, llm_result['error']
                        )
